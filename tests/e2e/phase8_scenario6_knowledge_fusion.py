"""
Phase 8 E2E Acceptance Execution — Scenario 6: HTTP Knowledge Fusion
"""
import sys
import uuid
import requests
...
def main():
    ...
    # 6. Create research mission that triggers knowledge fusion
    mission_resp = requests.post(
        ...
        json={
            ...
            "payload": {
                "query": "ما هي المتطلبات الجمركية لتصدير الخضروات المصرية الى الاردن",
    ...
    # 7. Validate business answer structure for knowledge fusion scenario
    ...
    # 8. Assertions per AC for knowledge fusion scenario
    failures = []
    ...
    # For knowledge fusion, we expect multiple sources or evidence from different domains
    # This indicates fusion of external research with knowledge providers
    has_multiple_sources = len(sources) > 1
    has_evidence_from_research = any(
        e.get("source_id") in ["un-comtrade", "worldbank-lpi"] for e in evidence
    )

    print(f"   Multiple sources: {has_multiple_sources}")
    print(f"   Evidence from external research: {has_evidence_from_research}")

    # Knowledge fusion is considered successful if:
    # 1. Business answer is present with valid structure
    # 2. Evidence exists from research sources
    # 3. No fabricated claims (evidence matches sources)
    if evidence and not sources:
        failures.append("AC-13: evidence exists but no sources")

    # Check for evidence-source consistency
    for ev in evidence:
        source_id = ev.get("source_id")
        if source_id and source_id not in sources:
            failures.append(f"AC-13: evidence source '{source_id}' not in sources list")
    ...
    print("\n[PASS] All assertions passed for Scenario 6")
    return True
