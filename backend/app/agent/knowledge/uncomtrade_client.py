...
class UnComtradeApiClient:
    """Isolated HTTP client for UN Comtrade API.

    This client is intentionally kept outside the DEM core and outside
    ``KnowledgeProvider``. It is owned by the UN Comtrade adapter boundary and
    ...
    async def request(
        ...
        if not path:
            raise ValueError("path is required for UN Comtrade API request")
