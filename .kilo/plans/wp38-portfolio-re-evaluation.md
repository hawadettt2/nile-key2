# External Source Portfolio Re-Evaluation — Strategic Review

**Date:** 2026-08-14  
**Scope:** All 20 External Source Candidates in `.kilo/plans/1786359213310-real-external-source-integration.md`  
...
|--------|-------------------|-------------------|-------------------|
| WP-38a | 1–5 | Moaah API | Egypt Customs, WTO TFA, WTO Tariff, World Bank WITS |
| WP-38b | 6–10 | TradeData API, NBD Trade Data, PST.AG, The Trade Hub | USITC HTS |
| WP-38c | 11–15 | ZATCA Open Data APIs | Jordan Trade Portal, Jordan Customs, UAE ICP, Saudi ZATCA Tariff |
| WP-38d | 16–20 | GCC-Stat | Qatar Customs, Kuwait Customs, Oman Customs, Bahrain Customs |
...
|---------------------|----------|-------------|-----|
| Trade Statistics | ✅ Yes | TradeData, GCC-Stat | None |
| Tariffs / Market Access | ⚠️ Partial | Moaah (duty rates), TradeData (HS codes), ZATCA (tariff via customs) | No dedicated tariff database |
| Customs / Regulations | ✅ Yes | Moaah (global), ZATCA (Saudi), GCC-Stat (aggregates) | Limited country-specific depth beyond Saudi |
| SPS / TBT / Product Requirements | ❌ No | None | **Gap** |
| Market Opportunity / Demand | ⚠️ Partial | TradeData (shipment records), GCC-Stat (economic indicators) | No dedicated market opportunity database |
| Competitor / Market Intelligence | ⚠️ Partial | TradeData (company profiles) | Limited competitor analysis |
| Provenance / Officiality | ✅ Yes | All 4 providers provide official or aggregated official data | None |
| Freshness / Update Frequency | ⚠️ Variable | TradeData (periodic), GCC-Stat (periodic), ZATCA (periodic), Moaah (periodic) | No real-time feeds |
| Country-specific Primary Sources | ⚠️ Limited | Saudi (ZATCA), GCC (GCC-Stat), Global (Moaah, TradeData) | 16+ countries uncovered |
...
**Overall Portfolio Coverage:** ~5/10 — functional but incomplete.
...
|--------|------|----------|------------------|-------------|
| **ITC Trade Map** | Tariff & market access | 200+ countries | Higher than current tariff coverage | Medium — requires subscription/API |
| **WTO Tariff Database** | Bound/applied tariffs | WTO members | Higher than aggregated sources | Medium — REST API available |
| **UN Comtrade** | Trade statistics | 200+ countries | Higher than TradeData for official stats | High — free API, SDMX/JSON |
| **WITS (World Bank)** | Trade & tariff data | Global | Similar to TradeData but official | Medium — API key required |
| **WTO TFA Database** | Trade facilitation | WTO members | Unique; no current coverage | High — free API available |
| **Global Trade Helpdesk** | Market access requirements | Global | Unique regulatory guidance | Low — primarily web UI |
| **UNCTADstat** | Trade & development stats | Global | Higher than GCC-Stat for global | Medium — SDMX API |
| **E-PING (ITC)** | SPS/TBT notifications | WTO members | **Fills critical SPS/TBT gap** | Medium — requires scraping/API |
...
1. **Architectural diminishing returns:** Each additional provider adds ~20–25 tests, ~2–3 files, and maintenance burden. After 4 providers, the pattern is proven; more providers add coverage but not architectural value.
...
- WTO Tariff & Trade Data (source 4) — complex redistribution terms; WTO TFA Database is more actionable
- World Bank WITS (source 5) — similar to TradeData; lower priority if TradeData is already integrated
- PST.AG (source 8) — enterprise sales model; overkill for DEM's current scope
- The Trade Hub (source 9) — EU-only; limited value for GCC-focused DEM

### 4.3 Which Missing Sources Should Be Added?

**High Priority:**
1. **WTO TFA Database** (tfadatabase.org) — Free API, high officiality, unique trade facilitation data
2. **UN Comtrade** — Free API, official global trade statistics, stronger than TradeData for official data
3. **E-PING (ITC)** — Fills critical SPS/TBT gap; high value for export compliance

**Medium Priority:**
4. **WTO Tariff Database** — Dedicated tariff data; complements Moaah
5. **ITC Trade Map** — Market access data; higher value than current tariff coverage
...
3. **PLAN.md priorities:** Phase 3 focuses on Docker Compose, Documentation, Owner Acceptance — not provider expansion.
...
**Yes — 4–6 providers maximum.** Reasoning:
...
1. **13 of 20 remaining sources are Tier B** (web-only, no documented REST APIs) — out of scope for standard pattern
2. **4 implemented providers already prove the pattern** — architectural goal achieved
...
**A) Approve Portfolio Revision** — Add WTO TFA Database, UN Comtrade, and E-PING as next providers; deprioritize all Tier B sources.

**B) Keep Current Portfolio** — Continue with existing 4 providers; defer additional providers indefinitely.

**C) Other** — Specify alternative direction.
...
*Review Status: Complete — Ready for Project Owner Decision*
