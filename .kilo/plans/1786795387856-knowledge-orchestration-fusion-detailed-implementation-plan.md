# Knowledge Orchestration / Fusion Layer — Detailed Implementation Plan

**Date:** 2026-08-15  
**Authority:** `.kilo/plans/1786795387856-knowledge-orchestration-fusion-plan.md` — Single Source of Truth  
**Plan Path:** `.kilo/plans/1786795387856-knowledge-orchestration-fusion-detailed-implementation-plan.md`  
**Status:** Detailed Planning Complete — Ready for Implementation Authorization  
...
## 2. orchestrator.py — Exact Structure
...
│   ├── __init__(self, registry, config)
│   ├── orchestrate(self, query, context, scope, sources, limit)  → Dict[str, Any]
│   ├── _classify_query(self, query)  → str
│   ├── _route_providers(self, query_type, sources_filter)  → List[Tuple[str, bool]]
│   ├── _query_providers(self, providers, query, context, limit)  → List[Dict[str, Any]]
│   ├── _compute_composite_score(self, result, provider_meta, query_type, is_primary)  → float
│   ├── _rank_results(self, results, provider_meta_map)  → List[Dict[str, Any]]
│   ├── _deduplicate(self, results)  → List[Dict[str, Any]]
│   ├── _detect_conflicts(self, results)  → List[Dict[str, Any]]
│   ├── _resolve_conflicts(self, results)  → List[Dict[str, Any]]
│   └── _build_orchestration_meta(self, ...)  → Dict[str, Any]
```
...
### 2.4 `_classify_query()` — Deterministic Algorithm
...
### 2.5 `_route_providers()` — Primary/Secondary Logic
...
**Return:** `List[Tuple[source_id, is_primary]]` — preserves primary/secondary distinction for downstream ranking.

### 2.6 `_query_providers()` — Parallel Query Execution
...
### 2.7 `_compute_composite_score()` — Ranking Formula
...
### 2.8 `_rank_results()` — Sorting and Limiting
...
### 2.9 `_deduplicate()` — Cross-Provider Dedup
...
### 2.10 `_resolve_conflicts()` — Conflict Detection and Resolution
...
def _resolve_conflicts(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for group in groups.values():
        ...
        if len(unique_dates) == 1 and len(unique_authorities) == 1:
            # Same date, same authority — keep both and flag conflict per planning spec
            for r in group:
            ...
            # True conflict — keep winner, flag others
...
### 2.11 `_build_output()` — Final Assembly
...
### 2.12 `_build_provider_meta_map()` — Metadata Lookup
...
## 3. engine.py Changes — Exact Diffs
...
## 5. config.py Changes — Exact Diff
...
4. _map_intent_to_candidates(intent, parameters) → mission candidates
5. _query_memory(session_id, intent) → memory entries
6. _query_knowledge(intent, parameters):
   ...
   b. Call orchestrator.orchestrate(query=intent, context=parameters, limit=10):
      i. _classify_query(intent) → query_type (e.g., "trade_statistics")
      ii. _route_providers(query_type, sources_filter) → [(source_id, is_primary), ...]
      iii. _query_providers(providers, query, context, limit) → raw_results
      iv. _build_provider_meta_map() → provider_meta
      v. _rank_results(raw_results, provider_meta, query_type) → ranked_results
      vi. _deduplicate(ranked_results) → deduped_results
      vii. _resolve_conflicts(deduped_results) → resolved_results
      viii. _build_output(resolved_results, query_type, providers, limit) → {
              "results": [...],
...
1-5. Same as above
6. _query_knowledge(intent, parameters):
   a. orchestrator = getattr(self, "_knowledge_orchestrator", None) → None
   b. return await self._query_knowledge_legacy(intent, parameters)
      → byte-for-byte equivalent to current behavior
7-16. Same as above (no orchestration metadata in request_context)
```

---

## 7. Test Plan — Exact Distribution
...
| 5 | "جمارك تصريح" | intent | `customs` |
| 6 | "regulation law" | intent | `regulatory` |
| 7 | "قانون لائحة" | intent | `regulatory` |
| 8 | "market access duty" | intent | `market_access` |
| 9 | "requirement متطلبات" | intent | `market_access` |
...
| 12 | "import إحصائيات" | intent | `trade_statistics` |
| 13 | "origin certificate" | intent | `rules_of_origin` |
| 14 | "شهادة منشأ" | intent | `rules_of_origin` |
...
| # | Test Case | Expected |
|---|-----------|----------|
| 1 | agrifood → primary contains faostat | True |
| 2 | agrifood → secondary contains tradedata | True |
| 3 | customs → primary contains zatca, moaah | True |
| 4 | regulatory → primary contains moaah only | True |
| 5 | market_access → primary contains moaah only | True |
| 6 | market_access → secondary contains zatca, gccstat, tradedata | True |
| 7 | trade_statistics → primary contains uncomtrade, tradedata | True |
| 8 | general → all registered providers | True |
...
| 3 | Same score tie — effective_date DESC | Newer date wins |
| 4 | Same date tie — source_id ASC | Alphabetical wins |
| 5 | Missing confidence defaults to 0.0 | Handled |
| 6 | Missing effective_date → recency 0.5 | Handled |
| 7 | Unknown authority_level → 0.5 weight | Handled |
...
| 8 | Empty results → confidence=None | None |
...
| # | Test Case | Expected |
|---|-----------|----------|
| 1 | Orchestrator attached → _query_knowledge uses orchestrator | Orchestrated results |
| 2 | Orchestrator not attached → _query_knowledge uses legacy | Legacy results |
...
- [ ] Classification is deterministic: same input → same output across 1000 runs
- [ ] Overlap handling: "export tariff regulation" → `market_access` (not `trade_statistics`)
...
**Acceptance Criteria:**
- [ ] Exact duplicates (same source, content[:100], date) → 1 result kept
- [ ] Cross-source duplicates (different sources, same content[:100], same date) → 1 result kept (highest authority)
- [ ] Different dates → both retained (not deduplicated)
- [ ] Conflict detection flags conflicts correctly
- [ ] "Latest official wins" strategy applied deterministically
- [ ] Dedup disabled via config → all results retained
...
├── Stage 1.1: Classification + Routing
├── Stage 1.2: Query Execution
├── Stage 1.3: Ranking
├── Stage 1.4: Deduplication
└── Stage 1.5: Conflict Resolution
    ↓
Phase 2: engine.py Changes
├── Stage 2.1: Extract _query_knowledge_legacy()
├── Stage 2.2: Replace _query_knowledge() body
└── Stage 2.3: Add orchestration metadata to reason()
    ↓
Phase 3: main.py + config.py
├── Stage 3.1: Add config settings
└── Stage 3.2: Wire orchestrator in lifespan()
    ↓
Phase 4: Tests (parallel with Phases 1-3)
├── Stage 4.1: Unit tests (66)
├── Stage 4.2: Integration tests (18)
└── Stage 4.3: Regression verification
    ↓
Phase 5: Verification
├── Stage 5.1: All tests pass
├── Stage 5.2: No regressions
└── Stage 5.3: Acceptance criteria met
```
...
|------------|--------|----------|
| No new Knowledge Families | ✅ | Section 1.3 Out of Scope |
| No new Providers | ✅ | Section 1.3 Out of Scope |
| No DEM core modifications beyond `_query_knowledge()` | ✅ | Section 1.3, 8.2 |
| No Knowledge Graph schema changes | ✅ | Section 1.3 Out of Scope |
| No database migrations | ✅ | Section 1.3 Out of Scope |
| No frontend changes | ✅ | Section 1.3 Out of Scope |
| No Avatar/Renderer changes | ✅ | Section 1.3 Out of Scope |
| No LLM summary in MVP | ✅ | Section 7.1 |
| `logistics` not a query type | ✅ | Section 2.1 Note |
| Moaah primary for market_access | ✅ | Section 3.1 |
| Legacy fallback preserved | ✅ | Section 8.2, 3.1 |
| orchestration in Decision.context | ✅ | Section 8.3 |
| Deterministic classification | ✅ | Section 2.2 |
...
1. `_query_knowledge()` exists at lines 336-379 of `engine.py` — **verified**
2. `Decision.context` is a plain dict — **verified** (Pydantic `Dict[str, Any]`)
3. `main.py` lifespan has provider registration before `yield` — **verified**
4. `config.py` uses Pydantic Settings with defaults — **verified**
5. `registry.list_providers()` returns list of source dicts — **verified**
6. `registry.query(source_id, ...)` returns dict with `results` key — **verified**
7. All providers return `source_id` in result items — **verified** (mooadapter, tradedata, zatca, gccstat, faostat, uncomtrade all set `source_id`)

---

*Plan Status: Detailed Implementation Plan Complete — Ready for Implementation Authorization — Next Step: Code Execution (not authorized in this mode)*
