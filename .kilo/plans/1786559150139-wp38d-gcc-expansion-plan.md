# WP-38d Plan: GCC Expansion — First Provider Implementation

**Work Package:** WP-38d — GCC Expansion (GCC-Stat First Provider)  
**Status:** Closed — Task 8 Completed — Ready for G5 Review  
**Date:** 2026-08-14  
**Authority:** `.kilo/plans/1786359213310-real-external-source-integration.md` Section 5.4, Section 14, Section 15  
...
**Path:** `.kilo/plans/1786559150139-wp38d-gcc-expansion-plan.md`
...
| Decision | Value |
|----------|-------|
| WP-38a Status | **Closed** — `baseline-wp38a-final` at commit `13fb461b` |
| WP-38b Status | **Closed** — `baseline-wp38b-final` at commit `02bad55` |
| WP-38c Status | **Closed** — `baseline-wp38c-final` at commit `e10295d` |
| WP-38d Status | **Approved** — G0 Approved — Ready for Task 1 |
| First Provider Candidate | **GCC-Stat** (`gccstat.org`) — Proposed for WP-38d Task 1 evaluation |
...
| Item | Description |
|------|-------------|
| Sources 16–20 evaluation | Evaluate all five sources against portfolio criteria during Task 1 |
...
| Item | Description |
|------|-------------|
| Multiple providers | Only one first provider is implemented in WP-38d. Sources 17–20 remain future providers. |
...
**Important:** Only one first provider will be implemented in WP-38d. The remaining sources 17–20 are future providers requiring independent evaluation, approval, adapter design, implementation, verification, and baseline following the same standard integration pattern.
...
|-----------|----------|--------|
| Machine-to-machine access | REST / SDMX APIs claimed — availability requires verification | **Pending Verification** |
| Data → `KnowledgeProvider.query()` shape | Trade statistics, economic indicators, customs aggregates | **Pending Verification** |
...
| Commercial license | Free (open data) — internal use within terms | **Likely Approved** |
| Redistribution terms | GCC-Stat open data policy — requires verification | **Pending Verification** |
| GCC coverage | GCC-wide — covers Qatar, Kuwait, Oman, Bahrain | **Verified** |
| Rate limits | Unknown — not publicly documented | **Pending Verification** |
| SDMX parsing complexity | Medium — requires SDMX client library | **Technical Risk** |

**GCC-Stat G1 Verdict:** **G1 Approved**  
**Approval Basis:** Project Owner approval obtained for GCC-Stat as WP-38d First Provider per `.kilo/plans/wp38d-owner-acceptance-certificate.md`.

**Fallback:** If GCC-Stat G1 blockers cannot be resolved, evaluate Qatar Customs (Source 17) as alternative first provider for WP-38d per the unified comparison matrix in `.kilo/plans/1786359213310-real-external-source-integration.md`.
...
| WP-38b closed and baselined | ✅ Complete | `baseline-wp38b-final` at `02bad55` |
| WP-38c closed and baselined | ✅ Complete | `baseline-wp38c-final` at `e10295d` |
...
| `config.py` pattern | ✅ Existing | New settings added for each provider |
| GCC-Stat API availability | ⚠️ To be verified | Must be verified during Task 1 |
| GCC-Stat commercial license | ⚠️ Pending | Requires Project Owner approval at G1 |
...
## 7. Tasks (Standard Integration Pattern — Reused from WP-38a/WP-38b/WP-38c)
...
- Evaluate sources 16–20 against portfolio criteria: authority/provenance, regulatory relevance, machine-to-machine access, cost/licensing, rate limits, reliability, freshness, coverage, integration complexity.
...
- Define transformation rules from selected provider response format → `KnowledgeProvider.query()` return shape
...
|------|-------------|-------------------|
| **G0 — WP-38d Plan Approval** | Project Owner approves WP-38d plan | WP does not start |
| **G1 — Source Selection** | First provider selected and G1 blockers resolved; Project Owner approves provider | WP-38d stops; no implementation begins |
| **G2 — Adapter Review** | Adapter spec reviewed and approved | WP-38d stops; implementation does not begin |
| **G3 — Implementation Review** | Code review confirms Provider-Agnostic architecture and contract compliance | WP-38d returns to Task 3 for fixes |
| **G4 — Verification** | All tests pass, no regressions, evidence complete | WP-38d returns to fix blockers |
| **G5 — Closure** | All acceptance criteria met, baseline tagged | WP-38d remains open |
...
| AC-38d.4 | `get_sources()` returns valid metadata with provenance | Unit test | — |
| AC-38d.5 | `query()` transforms provider data to contract shape | Unit test | — |
| AC-38d.6 | Confidence scores are within 0.0–1.0 per Task 2 rules | Unit test | — |
| AC-38d.7 | Provider registers successfully in `KnowledgeProviderRegistry` | Integration test | — |
| AC-38d.8 | Provider is queryable via registry without DEM core changes | Integration test | — |
| AC-38d.9 | `ReasoningEngine` can query provider through registry | Integration test | — |
| AC-38d.10 | Graceful degradation when provider is unavailable | Integration test | — |
...
2. Proceed to WP-38d Task 1 — Source Evaluation & Access Verification
3. Resolve remaining verification items (GCC-Stat API availability, SDMX parsing, rate limits) during Task 1
4. Project Owner approval of first provider — Required at G1
5. Proceed through Tasks 2–8 following the standard integration pattern
...
- **One provider at a time:** WP-38d integrates exactly one first provider. Remaining sources 17–20 are future providers requiring independent evaluation, approval, and implementation.
...
*Plan Status: Approved — G0 Approved — Ready for Task 1*
