# WP-37: Knowledge Ingestion Pipeline â€” File-based Regulations Ingestion Provider

**Work Package:** WP-37 â€” Knowledge Ingestion Pipeline (Phase 2.1)  
...
**Date:** 2026-08-10  
**Plan Path:** `\.kilo/plans/archive/1786359213310-knowledge-ingestion-pipeline\.md`

---

## 1. Architectural Intent

Implement the first concrete Knowledge Ingestion Provider that reads local regulation files (JSON), transforms them into the `KnowledgeProvider.query()` return shape, and registers them in the existing `KnowledgeProviderRegistry` â€” without modifying DEM core, Knowledge Graph schema, Memory, Research, or any existing contracts.
...
|-----------|-------|----------|
| `KnowledgeProvider` ABC | âœ… Exists | `backend/app/agent/knowledge/provider.py` |
| `KnowledgeProviderRegistry` | âœ… Exists | `backend/app/agent/knowledge/registry.py` |
| `CompanyKnowledgeProvider` | âœ… Exists | `backend/app/agent/knowledge/company_knowledge_provider.py` â€” queries `resources` table |
| `KnowledgeGraphProvider` | âœ… Exists | `backend/app/agent/knowledge/graph_provider.py` â€” queries `knowledge_nodes` |
| ReasoningEngine integration | âœ… Exists | `backend/app/agent/decision_engine/engine.py` â€” accepts `knowledge_provider_registry` |
| Bootstrap wiring | âœ… Exists | `backend/main.py` â€” registers both providers at startup |
| Research (WP-34) | âœ… Separate | `backend/app/research/orchestrator.py` â€” 7-stage lifecycle, independent |
| Memory (WP-31) | âœ… Separate | `backend/app/agent/memory/sqlite_provider.py` â€” no coupling to ingestion |
| `config.py` pattern | âœ… Existing | `backend/app/core/config.py` â€” Pydantic Settings with env vars; `REGULATIONS_FILE_PATH` absent but pattern established |
...
4. Assigns confidence scores (0.0â€“1.0) based on configurable rules
...
- Transform logic mapping file records â†’ `query()` return shape
...
### Decision 1: Regulation File Format â€” JSON
...
### Decision 2: File Location â€” Configurable Setting with Default
...
### Decision 3: Initial Data File â€” Test Fixture Created as Part of Task 1
...
| `CompanyKnowledgeProvider` | âœ… Existing | Reference implementation only |
| `KnowledgeGraphProvider` | âœ… Existing | Reference implementation only |
| `backend/main.py` bootstrap | âœ… Existing | Will add one `await registry.register(RegulationsKnowledgeProvider(...))` call |
| `backend/app/core/config.py` | âœ… Existing | Will add `REGULATIONS_FILE_PATH` setting |
| `backend/data/` directory | âœ… Existing | Default file location |
| Test fixture | âڑ ï¸ڈ Created in Task 1 | `backend/tests/fixtures/regulations.json` â€” test/reference data only |
...
- Create `backend/tests/fixtures/regulations.json` with 3â€“5 sample records covering:
...
- Test cases:
  ...
  5. Confidence scores within 0.0â€“1.0 range
...
| AC-37.4 | Confidence scores are within 0.0â€“1.0 | Unit test |
...
- **LLM Master Roadmap discrepancy:** `\.kilo/plans/archive/1786063180198-master-roadmap-remaining-phases\.md` lists "LLM Provider Integration" as "deferred", while `ENGINEERING_MEMORY.md` and git history confirm it is completed via WP-LLM-001. This document is out of date but is not addressed in WP-37 scope.
