# Export Readiness Vertical Slice â€” Implementation Plan

**Date:** 2026-08-20  
**Authority:** `.kilo/plans/1786559160142-external-knowledge-portfolio-re-evaluation.md` Section 32  
**Plan Path:** `\.kilo/plans/archive/1787046369933-export-readiness-vertical-slice\.md`  
**Status:** Implemented â€” Verified â€” Closed  
...
|------------|----------|--------|----------|
| Knowledge Orchestrator | `backend/app/agent/knowledge/orchestrator.py` | âœ… Implemented & Verified (77/77 tests) | Yes |
| Reasoning Engine | `backend/app/agent/decision_engine/engine.py` | âœ… Implemented | Yes |
| LLM (Gemini) | `backend/app/agent/llm/provider.py` | âœ… Implemented | Yes |
| Moaah External Source Adapter | `backend/app/agent/knowledge/mooadapter.py` | âœ… Implemented | Yes |
| ZATCA External Source Adapter | `backend/app/agent/knowledge/zatcaprovider.py` | âœ… Implemented | Yes |
| GCC-Stat External Source Adapter | `backend/app/agent/knowledge/gccstatprovider.py` | âœ… Implemented | Yes |
| UN Comtrade External Source Adapter | `backend/app/agent/knowledge/uncomtradeprovider.py` | âœ… Implemented | Yes |
| World Bank LPI External Source Adapter | `backend/app/agent/knowledge/worldbanklprovider.py` | âœ… Implemented | Yes |
| FAOSTAT External Source Adapter | `backend/app/agent/knowledge/faostatprovider.py` | âœ… Implemented | Yes |
| TradeData External Source Adapter | `backend/app/agent/knowledge/tradedataprovider.py` | âœ… Implemented | Yes |
| Knowledge Graph | `backend/app/agent/knowledge/graph_provider.py` | âœ… Implemented | Yes |
| DEM Mission Infrastructure | `backend/app/routers/digital_export_manager.py` | âœ… Implemented | Yes |
| Frontend Layout/Routing/Auth | `frontend/src/` | âœ… Implemented | Yes |
| DEM Landing Page | `frontend/src/pages/DEMLanding.tsx` | âœ… Implemented | Yes |
| DEM Mission Composer | `frontend/src/pages/DEMMissionComposer.tsx` | âœ… Implemented | Yes |
| DEM Mission Detail | `frontend/src/pages/DEMMissionDetail.tsx` | âœ… Implemented | Yes |

### 1.2 Confirmed Missing Capabilities

| Gap | Location | Status |
|-----|----------|--------|
| `/export-readiness` frontend page | `frontend/src/pages/` | â‌Œ Missing |
| Export Readiness API endpoint | `backend/app/routers/` | â‌Œ Missing |
| Report assembly service | `backend/app/services/` | â‌Œ Missing |
| EXPORT_READINESS mission type | `backend/app/agent/schemas/enums.py` | â‌Œ Missing (mission types are ERP-only) |
...
Current flow:
```
create_mission â†’ reasoning_engine.reason() â†’ task_planner.plan() â†’ execution_planner.plan() â†’ tool_orchestrator.execute()
```
...
| Tool execution | Yes (shipping, customs, etc.) | No â€” read-only knowledge queries |
| LLM integration | No | Yes â€” for recommendation synthesis |
| Report assembly | No | Yes â€” multi-source aggregation |
...
|----------------|-------------------|--------------|----------|
| **Regulatory Requirements** | `orchestrator.orchestrate(query, context={"country": "<ISO2>"}, sources=["moaah", "zatca"], limit=10)` | â€” | If empty: retry with `sources=["moaah", "zatca", "gccstat"]` |
| **Market Access** | `orchestrator.orchestrate(query, context={"country": "<ISO2>", "hs_code": "<code>"}, sources=["moaah", "zatca", "tradedata"], limit=10)` | â€” | If empty: retry with `sources=["moaah", "zatca", "gccstat"]` |
| **Logistics Profile** | â€” | `registry.query("worldbank-lpi", query, context={"country": "<ISO2>"}, scope="LP.LPI.OVRL.XQ", limit=10)` | â€” |
| **Historical Trade Context** | `orchestrator.orchestrate(query, context={"country": "<ISO2>", "product": "<name>"}, sources=["un-comtrade", "tradedata"], limit=10)` | â€” | If empty: retry with `sources=["un-comtrade", "tradedata", "faostat"]` |
...
| **Regulatory Requirements** | Moaah, ZATCA via Orchestrator (explicit `sources=["moaah", "zatca"]`) | available if â‰¥1 provider returns data with confidence â‰¥ 0.5 |
| **Market Access Conditions** | Moaah, ZATCA, TradeData via Orchestrator (explicit `sources=["moaah", "zatca", "tradedata"]`) | available if â‰¥1 provider returns data with confidence â‰¥ 0.5 |
...
| **Historical Trade Context** | UN Comtrade, TradeData via Orchestrator (explicit `sources=["un-comtrade", "tradedata"]`) | available if â‰¥1 provider returns data with confidence â‰¥ 0.5 |
...
| Label | Condition |
|-------|-----------|
| `available` | Data present, confidence â‰¥ 0.6, from â‰¥1 provider |
...
**Update:** `frontend/src/services/api.ts` â€” no changes needed if using separate service file
...
### Task 1: Backend â€” Export Readiness Service
...
### Task 2: Backend â€” Export Readiness Router
...
### Task 3: Frontend â€” Export Readiness Page
...
### Task 4: Frontend â€” Routing + Navigation
...
**E2E (if supported):**
- User enters product + market â†’ sees report
...
| LLM not configured (no API key) | Low | Graceful degradation â€” recommendation = null |
...
- â‌Œ No new providers
- â‌Œ No new research paths
- â‌Œ No external knowledge expansion
- â‌Œ No coverage score changes
- â‌Œ No provider ceiling changes
- â‌Œ No WP creation
- â‌Œ No Market Opportunity analysis
- â‌Œ No Global SPS/TBT coverage
- â‌Œ No avatar/renderer features
- â‌Œ No multi-market comparison
- â‌Œ No PDF export (v1 shows on-screen only)
...
**Task 1: Backend â€” Export Readiness Service + Router**

Create:
1. `backend/app/services/export_readiness.py` â€” Service with Pydantic models + `analyze()` method
2. `backend/app/routers/export_readiness.py` â€” Router with `POST /analyze` endpoint
...
*Plan Status: Ready for Implementation â€” Export Readiness Vertical Slice â€” Product Priority per Section 32 of External Knowledge Portfolio Re-Evaluation Plan*
