# Knowledge Orchestration / Fusion Layer â€” Planning Specification

**Date:** 2026-08-15  
**Authority:** `PLAN.md` v2.1 (Master Roadmap) â€” Single Source of Truth  
**Prerequisite:** `.kilo/plans/1786559160142-external-knowledge-portfolio-re-evaluation.md` â€” Governance Decision: Knowledge Orchestration/Fusion Layer is the next gap  
**Plan Path:** `\.kilo/plans/archive/1786795387856-knowledge-orchestration-fusion-plan\.md`  
**Status:** Planning Complete â€” Ready for Governance Review  
...
- **No synthesis** â€” downstream consumers receive raw merged lists
- **No routing** â€” irrelevant providers are queried unnecessarily
...
|------------|---------------------------|-----------------|---------------------|---------------------|
| `agrifood` | "agriculture", "food", "crop", "livestock", "ط²ط±ط§ط¹ط©", "ط؛ط°ط§ط،", "ظ…ط­طµظˆظ„", "ظ…ط§ط´ظٹط©" | `area`, `item`, `year` | FAOSTAT | UN Comtrade, TradeData |
| `customs` | "customs", "declaration", "HS code", "ط¬ظ…ط§ط±ظƒ", "طھطµط±ظٹط­ ط¬ظ…ط±ظƒظٹ", "ظƒظˆط¯ HS" | `hs_code`, `country` | ZATCA, Moaah | GCC-Stat |
| `regulatory` | "regulation", "law", "tariff", "ظ‚ط§ظ†ظˆظ†", "ظ„ط§ط¦ط­ط©", "طھط¹ط±ظٹظپط©" | `country`, `hs_code` | Moaah | ZATCA, GCC-Stat |
| `market_access` | "market access", "duty", "requirement", "ظپط±طµ ط³ظˆظ‚", "ظ…طھط·ظ„ط¨ط§طھ", "ط±ط³ظˆظ…" | `country`, `hs_code` | Moaah | ZATCA, GCC-Stat, TradeData |
| `trade_statistics` | "trade", "export", "import", "ط¥ط­طµط§ط¦ظٹط§طھ طھط¬ط§ط±ظٹط©", "طµط§ط¯ط±ط§طھ", "ظˆط§ط±ط¯ط§طھ" | `country`, `year`, `hs_code` | UN Comtrade, TradeData | GCC-Stat, FAOSTAT |
| `rules_of_origin` | "origin", "certificate", "ظ‚ط§ظ†ظˆظ† ط§ظ„ظ…ظ†ط´ط£", "ط´ظ‡ط§ط¯ط© ظ…ظ†ط´ط£" | `country`, `fta` | GCC-Stat | TradeData |
| `general` | *(no match)* | â€” | All registered | â€” |
...
### 2.2 Classification Rules â€” Deterministic Order
...
**Classification outcome:** First matching type wins by list order. If no match â†’ `general`.

**Overlap handling:**
- `customs` is checked before `market_access` and `regulatory` because customs-specific keywords (`customs`, `declaration`, `HS code`, `ط¬ظ…ط§ط±ظƒ`, `طھطµط±ظٹط­ ط¬ظ…ط±ظƒظٹ`, `ظƒظˆط¯ HS`) are more specific than generic regulatory or market access keywords.
...
- Example: "export tariff regulation" â†’ matches `market_access` first (keyword: "tariff"), not `regulatory` or `trade_statistics`.
...
| `general` | All registered | â€” | No preference â€” query all |
...
```python
{
    "id": str,              # Required â€” existing
    "name": str,            # Required â€” existing
    "type": str,            # Required â€” existing
    "authority_level": str, # Optional â€” "official" | "commercial" | "aggregated" (default: "aggregated")
    "coverage": [str],      # Optional â€” list of query types (default: ["general"])
    "version": str,         # Required â€” existing
    "updated_at": str,      # Required â€” existing
}
```
...
```
composite_score = (provider_confidence أ— 0.4)
                + (authority_weight أ— 0.3)
                + (recency_weight أ— 0.2)
                + (relevance_weight أ— 0.1)
```

Where:
- **provider_confidence**: `result["confidence"]` (0.0â€“1.0) from the provider
...
1. **Same content[:100] + same effective_date** â†’ duplicate group.
2. Within each duplicate group, if all results come from the **same source** â†’ exact duplicate â†’ keep highest `composite_score`.
3. Within each duplicate group, if results come from **different sources** â†’ cross-source duplicate â†’ keep highest `authority_weight` (`official > commercial > aggregated`). If authority is equal, keep highest `composite_score`.
4. **Different effective_date** â†’ NOT a duplicate â€” both retained for conflict resolution in Section 6.
...
| Different sources, different dates | Keep **latest date** if authority difference â‰¤ 1 level; otherwise keep higher authority |
| Equal authority, equal date | Keep both â€” flag as `conflict: true` |
| No `effective_date` on either | Keep both â€” flag as `conflict: true` |
...
## 7. Knowledge Synthesis (MVP â€” List Mode Only)
...
class KnowledgeOrchestrator:
    ...
    async def orchestrate(
        ...
        """Execute full orchestration pipeline: classify â†’ route â†’ query â†’ rank â†’ dedup â†’ resolve conflicts."""
```
...
**Change:** Replace body of `_query_knowledge()` (lines 336â€“379) with orchestrator call.
...
**Legacy fallback:** `_query_knowledge_legacy()` is the exact current implementation extracted to a separate method. Its behavior is byte-for-byte equivalent to the current code at lines 336â€“379 of `engine.py`. It exists solely for fallback when the orchestrator is not attached.
...
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک
                                     ...
                                     â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ
â”‚                        ReasoningEngine.reason()                          â”‚
â”‚                                                                           â”‚
â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ  â”‚
â”‚  â”‚  _query_knowledge(intent, parameters)                               â”‚  â”‚
â”‚  â”‚                                                                       â”‚  â”‚
â”‚  â”‚  1. Retrieve _knowledge_orchestrator if attached                      â”‚  â”‚
â”‚  â”‚  2. If attached: call orchestrator.orchestrate(...)                  â”‚  â”‚
â”‚  â”‚  3. If not attached: call _query_knowledge_legacy(...)               â”‚  â”‚
â”‚  â”‚  4. Return results list                                               â”‚  â”‚
â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک  â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک
                                     â”‚
                                     â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ
â”‚                     KnowledgeOrchestrator (new)                          â”‚
â”‚                                                                           â”‚
â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ  â”‚
â”‚  â”‚  1. Classifyâ”‚â”€â–¶â”‚ 2. Route     â”‚â”€â–¶â”‚ 3. Query     â”‚â”€â–¶â”‚ 4. Rank      â”‚  â”‚
â”‚  â”‚  Query      â”‚  â”‚ Providers    â”‚  â”‚ Providers    â”‚  â”‚ Results      â”‚  â”‚
â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک  â””â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”ک  â”‚
â”‚                                                              â”‚          â”‚
â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ       â”‚          â”‚
â”‚  â”‚ 7. Return   â”‚â—€â”€â”‚ 6. Resolve   â”‚â—€â”€â”‚ 5. Dedup      â”‚â—€â”€â”€â”€â”€â”€â”€â”ک          â”‚
â”‚  â”‚ Orchestratedâ”‚  â”‚ Conflicts    â”‚  â”‚              â”‚                    â”‚
â”‚  â”‚ Results     â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک                    â”‚
â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک                                                        â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک
                                     â”‚
                    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ
                    â–¼                                 â–¼
      â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ      â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ
      â”‚ KnowledgeProviderRegistryâ”‚      â”‚   LLM Registry (optional)â”‚
      â”‚  .list_providers()       â”‚      â”‚   â€” NOT used in MVP      â”‚
      â”‚  .query(source_id, ...)  â”‚      â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک
      â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک
                 â”‚
    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ
    â–¼            â–¼            â–¼
 â”Œâ”€â”€â”€â”€â”€â”€â”€â”گ  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”گ
 â”‚ Moaah â”‚  â”‚ TradeDataâ”‚  â”‚  ZATCA   â”‚ ...
 â””â”€â”€â”€â”€â”€â”€â”€â”ک  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”ک
```
...
This ensures **backward compatibility** â€” existing providers work without modification.
...
| Database migrations | Not required â€” orchestration is computed per-query |
...
1. **Create `orchestrator.py`** â€” `KnowledgeOrchestrator` class with classification, routing, ranking, dedup, conflict resolution
2. **Add config settings** â€” `config.py` new section
3. **Extract `_query_knowledge_legacy()`** â€” Extract current lines 336â€“379 of `engine.py` to a separate method
4. **Modify `_query_knowledge()`** â€” Replace body with orchestrator call, preserve legacy fallback
5. **Wire in `main.py`** â€” Instantiate orchestrator and attach to `ReasoningEngine`
6. **Write unit tests** â€” ~65 tests covering all orchestration logic
7. **Write integration tests** â€” ~18 tests covering end-to-end and ReasoningEngine integration
8. **Run regression tests** â€” Verify zero regressions in existing test suites
9. **G4/G5 Verification** â€” Test plan execution and closure
...
| Classification overlap | Fixed priority order: agrifood â†’ customs â†’ regulatory â†’ market_access â†’ trade_statistics â†’ rules_of_origin | Most specific keywords first; deterministic |
...
|---|------|------|-----------------|
| 1 | **Moaah `market_access` coverage** â€” confirm Moaah provides duty/tariff data for markets beyond Egypt | Validation Item | Verify Moaah API response fields; if Egypt-only, adjust routing to ZATCA primary for non-Egypt contexts |
| 2 | **UN Comtrade `effective_date` format** â€” confirm date format consistency for ranking and dedup | Validation Item | Verify UN Comtrade preview API returns parseable dates; if inconsistent, add date parsing fallback |
| 3 | **TradeData confidence values** â€” confirm 0.50â€“0.95 range is compatible with composite score formula | Validation Item | Verify composite_score stays within 0.0â€“1.0 bounds with current provider confidence ranges |
| 4 | **`content[:100]` slice length for dedup** â€” confirm 100 chars is sufficient for stable dedup keys | Engineering Decision | If 100 chars causes false negatives, increase to 200; if too long, decrease to 50 |
...
| 3 | TradeData confidence range compatibility | Composite score must remain within 0.0â€“1.0 with all provider confidence inputs | **Pending implementation-time verification** |
...
*Plan Status: Complete â€” Ready for Governance Adoption â€” Next Step: Detailed Implementation Plan (not code execution)*
