...
interface GraphEdgeProps { edge: ArchitectureEdge; sourcePos: GraphNodePosition | undefined; targetPos: GraphNodePosition | undefined; }
...
export function GraphEdge({ edge, sourcePos, targetPos }: GraphEdgeProps) {
  ...
  return (
    <g className="graph-edge" opacity={isConditional ? 0.95 : 0.82}>
      <path d={path} fill="none" stroke={strokeColor} strokeWidth={isConditional ? 2.4 : 1.8} strokeDasharray={isConditional ? '8 5' : undefined} strokeLinecap="round" markerEnd="url(#architecture-arrow)" />
    </g>
  );
}

export { NODE_WIDTH, NODE_HEIGHT, RELATION_COLORS };
