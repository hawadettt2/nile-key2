...
function InputOTPSlot({ index, className, ...props }: React.ComponentProps<"div"> & { index: number }) {
  ...
  return (
    <div data-slot="input-otp-slot" data-active={isActive} className={cn("border-input relative flex h-10 w-10 items-center justify-center border-y border-r text-sm transition-all first:rounded-l-md first:border-l last:rounded-r-md", isActive && "ring-ring z-10 ring-2", className)} {...props}>
...
function InputOTPSeparator({ ...props }: React.ComponentProps<"div">) {
  return <div data-slot="input-otp-separator" {...props}><Dot /></div>
}
